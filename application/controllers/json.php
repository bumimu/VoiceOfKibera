...
class Json_Controller extends Template_Controller
{
	...
	public function cluster()
    {
      ...
      if ($cluster_count > 10) {
        $cluster_list_count = 10;
      } else {
        $cluster_list_count = $cluster_count;
      }
      $cluster_name = "<ul>";
      for ($i=0; $i<$cluster_list_count; $i++) {
         $marker = $cluster[$i];
         $cluster_name .= "<li><a href=" . url::base() . "reports/view/" . $marker['id'] .">" . $marker['incident_title'] ."</a></li>";
      }
      $cluster_name .= "</ul>";
      if ($cluster_count == 11) {
        $cluster_name .= "and <a href=" . url::base() . "reports/index/?c=".$category_id."&sw=".$southwest."&ne=".$northeast.">" . 1 . " More Report</a>";
      } elseif ($cluster_count > 10) {     
        $cluster_name .= "and <a href=" . url::base() . "reports/index/?c=".$category_id."&sw=".$southwest."&ne=".$northeast.">" . $cluster_count . " More Reports</a>";
      }
        
			$json_item = "{";
		    $json_item .= "\"type\":\"Feature\",";
		    $json_item .= "\"properties\": {";
			$json_item .= "\"name\":\"" . str_replace(chr(10), ' ', str_replace(chr(13), ' ', $cluster_name)) . "\",";			
		    $json_item .= "\"category\":[0], ";
			$json_item .= "\"color\": \"".$color."\", ";
			$json_item .= "\"icon\": \"".$icon."\", ";
